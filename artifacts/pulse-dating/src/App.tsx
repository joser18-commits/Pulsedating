...
function AuthPage({ mode }: { mode: 'sign-in' | 'sign-up' }) {
  return <div className="noise flex min-h-[100dvh] items-center justify-center bg-background px-4 py-8"><div className="absolute left-5 top-6 md:left-8"><Logo /></div><div className="clerk-wrap w-full max-w-[440px]"><div className="mb-5 text-center"><p className="font-mono-pulse text-[11px] uppercase tracking-[.2em] text-primary">{mode === 'sign-in' ? 'Your signal is here' : 'Make the first move'}</p><p className="mt-2 text-sm text-muted-foreground">{mode === 'sign-up' ? 'PULSE is for adults 18+ building toward real connection.' : 'Welcome back to a more intentional kind of dating.'}</p></div>{mode === 'sign-in' ? <SignIn routing="path" path={`${basePath}/sign-in`} signUpUrl={`${basePath}/sign-up`} /> : <SignUp routing="path" path={`${basePath}/sign-up`} signInUrl={`${basePath}/sign-in`} />}</div></div>;
}
...
function AppShell({ children }: { children: ReactNode }) {
  ...
  const links = [{ href: '/app', label: 'Home', icon: Compass }, { href: '/vibe-dna', label: 'Vibe DNA', icon: Sparkles }, { href: '/profile', label: 'Profile', icon: UserRound }, { href: '/settings', label: 'Settings', icon: Settings2 }];
...
function SettingsPage() {
  ...
  const { signOut } = useClerk();
  const form = useForm<SettingsInput & { ageMin?: number; ageMax?: number; maxDistanceMiles?: number }>({ values: { ...data, ageMin: preferences.data?.ageMin, ageMax: preferences.data?.ageMax, maxDistanceMiles: preferences.data?.maxDistanceMiles }, defaultValues: { discoverable: true, showAge: true, showRegion: true, notificationsEnabled: true, deleteRequested: false, ageMin: 25, ageMax: 42, maxDistanceMiles: 50 } });
...
function ClerkRoutes() {
  ...
  return <ClerkProvider publishableKey={clerkPubKey} proxyUrl={clerkProxyUrl} appearance={appearance} signInUrl={`${basePath}/sign-in`} signUpUrl={`${basePath}/sign-up`} localization={{ signIn: { start: { title: 'Welcome back', subtitle: 'Return to a more intentional kind of dating' } }, signUp: { start: { title: 'Create your PULSE', subtitle: 'For adults 18+ ready for something real' } } }} routerPush={(to: string) => setLocation(stripBase(to))} routerReplace={(to: string) => setLocation(stripBase(to), { replace: true })}><QueryClientProvider client={queryClient}><ClerkCacheInvalidator /><RouterContent /></QueryClientProvider></ClerkProvider>;
}

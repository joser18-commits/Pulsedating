...
function ProfileCard({ profile, onSignal, onSuperPulse, signalPending, likeAllowance, heartAllowance }: { profile: DiscoveryProfile; onSignal: (kind: HeartInputKind) => void; onSuperPulse: () => void; signalPending: boolean; likeAllowance: { remaining: number; limit: number; refillAt: string }; heartAllowance: { remaining: number; limit: number; refillAt: string } }) {
  ...
  return <article className="overflow-hidden rounded-3xl border border-border bg-card shadow-2xl shadow-black/10" data-testid={`card-discovery-profile-${profile.userId}`}>
    ...
    <div className="p-5 md:p-6">
      ...
      <div className="mt-5 grid grid-cols-3 gap-2 border-t border-border pt-5"><button type="button" onClick={() => onSignal(HeartInputKind.like)} disabled={signalPending || likeAllowance.remaining <= 0} className="focus-ring inline-flex min-h-12 items-center justify-center gap-1 rounded-xl border border-secondary/60 text-xs font-semibold text-secondary transition hover:bg-secondary/10 disabled:opacity-45" data-testid={`button-like-${profile.userId}`}><ThumbsUp size={15} /> Like</button><button type="button" onClick={() => onSignal(HeartInputKind.heart)} disabled={signalPending || heartAllowance.remaining <= 0} className="focus-ring inline-flex min-h-12 items-center justify-center gap-1 rounded-xl bg-primary text-xs font-semibold text-primary-foreground transition hover:bg-[#FFE58A] disabled:opacity-45" data-testid={`button-heart-${profile.userId}`}><Heart size={16} fill="currentColor" /> Heart</button><button type="button" onClick={onSuperPulse} disabled={signalPending} className="focus-ring inline-flex min-h-12 items-center justify-center gap-1 rounded-xl border border-accent/60 text-xs font-semibold text-accent transition hover:bg-accent/10 disabled:opacity-45" data-testid={`button-super-pulse-${profile.userId}`}><Zap size={15} /> Super Pulse</button></div>
    </div>
  </article>;
}
...
export default function DiscoveryPage() {
  ...
  const heartAllowance = result?.heartAllowance ?? { remaining: 0, limit: 0, refillAt: '' };
  const likeAllowance = result?.likeAllowance ?? { remaining: 0, limit: 0, refillAt: '' };
  if (discovery.isPending && !result) return <LoadingDiscovery />;
  if (discovery.isError && !result) return <DiscoveryError retry={() => run()} />;
  return <DiscoveryFrame><div className="animate-rise"><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="mb-3 flex items-center gap-2 font-mono-pulse text-[11px] uppercase tracking-[.2em] text-primary"><Radio size={13} /> Discovery / your frequency</p><h1 className="font-display text-4xl font-semibold tracking-[-.06em] md:text-6xl">Find the <span className="text-primary">right signal.</span></h1><p className="mt-4 max-w-xl text-[15px] leading-7 text-muted-foreground">Explore real people by intention, not performance. Keep your pace and your boundaries.</p></div><div className="grid grid-cols-2 gap-2 rounded-2xl border border-primary/25 bg-primary/[.06] px-4 py-3"><div><p className="flex items-center gap-2 font-mono-pulse text-[10px] uppercase tracking-[.14em] text-secondary"><ThumbsUp size={13} /> Likes</p><p className="mt-1 font-display text-2xl">{likeAllowance.remaining} <span className="font-sans text-xs text-muted-foreground">/ {likeAllowance.limit}</span></p></div><div><p className="flex items-center gap-2 font-mono-pulse text-[10px] uppercase tracking-[.14em] text-primary"><Heart size={13} fill="currentColor" /> Hearts</p><p className="mt-1 font-display text-2xl">{heartAllowance.remaining} <span className="font-sans text-xs text-muted-foreground">/ {heartAllowance.limit}</span></p></div><p className="col-span-2 flex items-center gap-1 text-[11px] text-muted-foreground"><Clock3 size={12} /> {formatRefill(heartAllowance.refillAt)}</p></div></div>
